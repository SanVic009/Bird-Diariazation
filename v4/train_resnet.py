...
from v4.resnet import ResNetBird
import numpy as np
...
def train_resnet(train_loader, val_loader, n_classes, device="cuda", 
               patience=10, lr=1e-3, weight_decay=1e-4, 
               scheduler_type="cosine", max_epochs=50, save_dir="checkpoints_resnet"):

    global train_losses, val_losses, train_accuracies, val_accuracies

    # Log model type and training start
    logger.info(f"Starting ResNet training with {n_classes} classes")
    logger.info(f"Training parameters: epochs={max_epochs}, lr={lr}, device={device}")

    # Generate a timestamp for unique filenames
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")

    model = ResNetBird(n_classes=n_classes).to(device)
